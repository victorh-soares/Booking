...
const BookingItem = ({
  ...
  barbershopName: propBarbershopName,
  barbershopImageUrl: propBarbershopImageUrl,
  date: propDate,
}: BookingItemProps) => {
  const [sheetIsOpen, setSheetIsOpen] = useState(false);

  const booking = propBooking ?? {
    id: "",
    date: propDate ? new Date(propDate) : new Date(),
    cancelled: false,
    service: {
      name: propServiceName ?? "",
      priceInCents: 5000,
    },
    barbershop: {
      id: "",
      name: propBarbershopName ?? "",
      imageUrl: propBarbershopImageUrl ?? "",
      address: "",
      phones: [],
    },
  };
    ...
    <Sheet open={sheetIsOpen} onOpenChange={setSheetIsOpen}>
      ...
      <SheetContent className="w-[370px] overflow-y-auto p-0">
        ...
        <div className="space-y-6 px-5 py-6">
          ...
          {/* Telefones */}
          {(booking.barbershop.phones?.length ?? 0) > 0 && (
